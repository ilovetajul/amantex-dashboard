...
import {
  ...
  RefreshCw, FileSpreadsheet, Download, Link, Save, Loader,
} from "lucide-react";
...
const UploadScreen = ({onFile, onGSheet, dark, onToggle}) => {
  ...
  const [tab, setTab]         = useState("file"); // "file" | "gsheet"
  ...
  const loadGSheet = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("ফাইল লোড হয়নি। শিটটি Public করুন।");
      const buf = await res.arrayBuffer();
      onGSheet(buf, gsUrl);
    } catch(e) {
      setGsError(e.message || "লোড করতে সমস্যা হয়েছে।");
    }
    setLoading(false);
  };

  const bgPage = dark?"#030712":"#f1f5f9";
  const bgCard = dark?"rgba(15,23,42,0.75)":"rgba(255,255,255,0.85)";
  const border = "rgba(99,102,241,0.2)";

  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:bgPage,padding:20}}>
      <button onClick={onToggle} style={{position:"fixed",top:14,right:14,padding:"8px",borderRadius:10,border:`1px solid ${border}`,cursor:"pointer",background:"rgba(99,102,241,0.1)",color:dark?"#fbbf24":"#6366f1",display:"flex",alignItems:"center"}}>
      ...
      <div style={{maxWidth:440,width:"100%"}}>
        ...
        {/* Tab toggle */}
        <div style={{display:"flex",gap:4,background:dark?"rgba(30,41,59,0.8)":"rgba(241,245,249,0.9)",padding:4,borderRadius:14,marginBottom:14,border:`1px solid ${border}`}}>
          {[["file","📂 Excel আপলোড"],["gsheet","🔗 Google Sheets"]].map(([t,l])=>(
            <button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:"9px",borderRadius:10,border:"none",cursor:"pointer",fontSize:12,fontWeight:700,transition:"all 0.2s",background:tab===t?(dark?"rgba(99,102,241,0.9)":"#6366f1"):"transparent",color:tab===t?"#fff":dark?"#64748b":"#94a3b8"}}>
              {l}
            </button>
          ))}
        </div>
        ...
        {tab==="file" && (
          <>
            <div
              ...
              onDrop={e=>{setDrag(false);handleFile(e)}}
        ...
        {/* GOOGLE SHEETS TAB */}
        {tab==="gsheet" && (
          <div style={{background:bgCard,borderRadius:20,padding:"20px",border:`1px solid ${border}`,backdropFilter:"blur(20px)"}}>
            ...
            <textarea
              ...
              rows={3}
              style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`1px solid ${border}`,background:dark?"rgba(30,41,59,0.8)":"rgba(248,250,252,0.9)",color:dark?"#e2e8f0":"#0f172a",fontSize:12,resize:"none",outline:"none",boxSizing:"border-box",fontFamily:"monospace"}}
            />

            {gsError && (
              <div style={{background:"rgba(244,63,94,0.12)",border:"1px solid rgba(244,63,94,0.3)",borderRadius:8,padding:"8px 10px",marginTop:8,fontSize:11,color:"#f43f5e",fontWeight:600}}>
                ⚠️ {gsError}
              </div>
            )}
            ...
            <div style={{background:dark?"rgba(99,102,241,0.08)":"rgba(99,102,241,0.05)",borderRadius:10,padding:"10px 12px",marginTop:10,marginBottom:12}}>
              <p style={{fontSize:10,fontWeight:700,color:"#6366f1",margin:"0 0 4px",textTransform:"uppercase",letterSpacing:"0.05em"}}>📋 Google Sheets Public করার নিয়ম</p>
              <p style={{fontSize:11,color:dark?"#94a3b8":"#64748b",margin:0,lineHeight:1.6}}>
                Share → Anyone with the link → Viewer → Done
              </p>
            </div>

            {/* Buttons */}
            <div style={{display:"flex",gap:8}}>
              <button onClick={saveLink} style={{flex:1,padding:"10px",borderRadius:10,border:`1px solid ${saved?"rgba(16,185,129,0.4)":"rgba(99,102,241,0.3)"}`,cursor:"pointer",fontSize:12,fontWeight:700,background:saved?"rgba(16,185,129,0.15)":"rgba(99,102,241,0.12)",color:saved?"#10b981":"#818cf8",display:"flex",alignItems:"center",justifyContent:"center",gap:5,transition:"all 0.3s"}}>
                <Save size={13}/> {saved?"✓ সেভ হয়েছে!":"লিংক সেভ করুন"}
              </button>
              <button onClick={loadGSheet} disabled={!gsUrl||loading} style={{flex:1,padding:"10px",borderRadius:10,border:"none",cursor:gsUrl&&!loading?"pointer":"not-allowed",fontSize:12,fontWeight:700,background:gsUrl&&!loading?"linear-gradient(135deg,#6366f1,#8b5cf6)":"rgba(99,102,241,0.3)",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",gap:5,opacity:gsUrl&&!loading?1:0.6}}>
                {loading?<><Loader size={13} style={{animation:"spin 1s linear infinite"}}/> লোড হচ্ছে...</>:<><Link size={13}/> ডেটা লোড করুন</>}
              </button>
            </div>

            {/* Saved link indicator */}
            {localStorage.getItem("amantex_gsurl") && (
              <div style={{marginTop:10,padding:"8px 10px",background:dark?"rgba(16,185,129,0.08)":"rgba(16,185,129,0.06)",borderRadius:8,border:"1px solid rgba(16,185,129,0.2)"}}>
                <p style={{fontSize:10,color:"#10b981",fontWeight:600,margin:0}}>✅ সেভ করা লিংক আছে — লোড করতে উপরে ক্লিক করুন</p>
              </div>
            )}
          </div>
        )}
...
export default function App() {
  ...
  const handleGSheet = useCallback((arrayBuffer, url)=>{
    setError("");
    processWorkbook(arrayBuffer, "Google Sheets");
  },[]);
