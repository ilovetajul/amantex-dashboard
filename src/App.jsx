...
import {
  ...
  RefreshCw, FileSpreadsheet, Download, Link, Save, Loader, FileText,
} from "lucide-react";
...
const toCSVUrl = url => {
  ...
  return `https://docs.google.com/spreadsheets/d/${match[1]}/export?format=xlsx&id=${match[1]}`;
};
...
// ── UPLOAD SCREEN ─────────────────────────────────────────────────
const UploadScreen = ({onFile, onGSheet, dark, onToggle}) => {
  const [drag, setDrag]   = useState(false);
  const [tab, setTab]     = useState("file");
  const [gsUrl, setGsUrl] = useState(() => localStorage.getItem("amantex_gsurl")||"");
  ...
  const saveLink = () => {
    localStorage.setItem("amantex_gsurl", gsUrl);
    setSaved(true); setTimeout(()=>setSaved(false),2000);
  };
  const loadGSheet = async () => {
    setGsError(""); setLoading(true);
    const csvUrl = toCSVUrl(gsUrl);
    if (!csvUrl) { setGsError("সঠিক Google Sheets লিংক দিন।"); setLoading(false); return; }
    try {
      const res = await fetch(csvUrl);
      if (!res.ok) throw new Error("লোড হয়নি। শিটটি Public করুন।");
      onGSheet(await res.arrayBuffer(), gsUrl);
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:dark?"#030712":"#f1f5f9",padding:20}}>
      ...
      <div style={{maxWidth:440,width:"100%"}}>
        ...
        <div style={{display:"flex",gap:4,background:dark?"rgba(30,41,59,0.8)":"rgba(241,245,249,0.9)",padding:4,borderRadius:14,marginBottom:14,border:"1px solid rgba(99,102,241,0.2)"}}>
          {[["file","📂 Excel আপলোড"],["gsheet","🔗 Google Sheets"]].map(([t,l])=>(
            <button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:"9px",borderRadius:10,border:"none",cursor:"pointer",fontSize:12,fontWeight:700,background:tab===t?(dark?"rgba(99,102,241,0.9)":"#6366f1"):"transparent",color:tab===t?"#fff":dark?"#64748b":"#94a3b8"}}>
              {l}
            </button>
          ))}
        </div>
        ...
        {tab==="file" && (
          <>
            <div onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)}
              onDrop={e=>{setDrag(false);handleFile(e)}} onClick={()=>document.getElementById("xl").click()}
              style={{border:`2px dashed ${drag?"#6366f1":dark?"rgba(99,102,241,0.4)":"rgba(99,102,241,0.3)"}`,borderRadius:20,padding:"36px 24px",background:dark?"rgba(15,23,42,0.6)":"rgba(255,255,255,0.85)",backdropFilter:"blur(20px)",cursor:"pointer",textAlign:"center"}}>
              ...
              <p style={{fontWeight:800,fontSize:16,color:dark?"#e2e8f0":"#0f172a",marginBottom:4}}>Excel ফাইল আপলোড করুন</p>
              <p style={{fontSize:12,color:"#64748b",margin:0}}>ক্লিক করুন বা ড্র্যাগ করুন (.xlsx)</p>
            </div>
            <input id="xl" type="file" accept=".xlsx,.xls" style={{display:"none"}} onChange={handleFile}/>
          </>
        )}

        {tab==="gsheet" && (
          <div style={{background:dark?"rgba(15,23,42,0.75)":"rgba(255,255,255,0.85)",borderRadius:20,padding:"20px",border:"1px solid rgba(99,102,241,0.2)"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
              ...
              <div>
                <p style={{margin:0,fontSize:13,fontWeight:800,color:dark?"#e2e8f0":"#0f172a"}}>Google Sheets লিংক</p>
                <p style={{margin:0,fontSize:10,color:"#64748b"}}>শিটটি Public করা থাকতে হবে</p>
              </div>
            </div>
            <textarea value={gsUrl} onChange={e=>setGsUrl(e.target.value)}
              placeholder="https://docs.google.com/spreadsheets/d/..." rows={3}
              style={{width:"100%",padding:"10px 12px",borderRadius:10,border:"1px solid rgba(99,102,241,0.2)",background:dark?"rgba(30,41,59,0.8)":"rgba(248,250,252,0.9)",color:dark?"#e2e8f0":"#0f172a",fontSize:12,resize:"none",outline:"none",boxSizing:"border-box",fontFamily:"monospace"}}/>
            {gsError&&<div style={{background:"rgba(244,63,94,0.12)",border:"1px solid rgba(244,63,94,0.3)",borderRadius:8,padding:"8px",marginTop:8,fontSize:11,color:"#f43f5e",fontWeight:600}}>⚠️ {gsError}</div>}
            <div style={{background:dark?"rgba(99,102,241,0.08)":"rgba(99,102,241,0.05)",borderRadius:10,padding:"10px 12px",marginTop:10,marginBottom:12}}>
              <p style={{fontSize:10,fontWeight:700,color:"#6366f1",margin:"0 0 3px",textTransform:"uppercase",letterSpacing:"0.05em"}}>Public করার নিয়ম</p>
              <p style={{fontSize:11,color:dark?"#94a3b8":"#64748b",margin:0}}>Share → Anyone with the link → Viewer → Done</p>
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={saveLink} style={{flex:1,padding:"10px",borderRadius:10,border:`1px solid ${saved?"rgba(16,185,129,0.4)":"rgba(99,102,241,0.3)"}`,cursor:"pointer",fontSize:12,fontWeight:700,background:saved?"rgba(16,185,129,0.15)":"rgba(99,102,241,0.12)",color:saved?"#10b981":"#818cf8",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
                <Save size={12}/>{saved?"✓ সেভ হয়েছে!":"লিংক সেভ করুন"}
              </button>
              <button onClick={loadGSheet} disabled={!gsUrl||loading} style={{flex:1,padding:"10px",borderRadius:10,border:"none",cursor:gsUrl&&!loading?"pointer":"not-allowed",fontSize:12,fontWeight:700,background:gsUrl&&!loading?"linear-gradient(135deg,#6366f1,#8b5cf6)":"rgba(99,102,241,0.3)",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
                {loading?<><Loader size={12} style={{animation:"spin 1s linear infinite"}}/> লোড হচ্ছে...</>:<><Link size={12}/> ডেটা লোড করুন</>}
              </button>
            </div>
          </div>
        )}
...
export default function App() {
  ...
  // Refs for each section to capture
  const refKPI      = useRef();
  const refDaily    = useRef();
  const refGauge    = useRef();
  const refDefect   = useRef();
  const refPie      = useRef();
  ...
  const processWorkbook = (arrayBuffer, name) => {
    try {
      ...
      setMonth("all"); setFileName(name);
    } catch(err){setError("ফাইল পড়তে সমস্যা: "+err.message);}
  };

  const handleFile  = useCallback(file=>{setError("");const r=new FileReader();r.onload=e=>processWorkbook(e.target.result,file.name);r.readAsArrayBuffer(file);},[]);
  const handleGSheet= useCallback((buf,url)=>{setError("");processWorkbook(buf,"Google Sheets");},[]);
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      const sections = [
        { ref: refKPI,    title: "KPI Summary" },
        { ref: refDaily,  title: "Daily Production Overview" },
        { ref: refGauge,  title: "Performance Gauge" },
        { ref: refDefect, title: "Daily Defect Analysis" },
        { ref: refPie,    title: "Pass vs Reject Ratio" },
        { ref: refTable,  title: "Top 5 Reject Days" },
      ];
      ...
      // Logo box
      pdf.setFillColor(99, 102, 241);
      pdf.roundedRect(pdfW/2-14, 60, 28, 28, 4, 4, "F");
      pdf.setTextColor(255,255,255);
      pdf.setFontSize(20); pdf.setFont("helvetica","bold");
      pdf.text("A", pdfW/2, 78, {align:"center"});
      // Title
      pdf.setFontSize(22); pdf.setFont("helvetica","bold");
      pdf.setTextColor(226,232,240);
      pdf.text("Amantex Limited", pdfW/2, 105, {align:"center"});
      pdf.setFontSize(11); pdf.setFont("helvetica","normal");
      pdf.setTextColor(100,116,139);
      pdf.text("QUALITY ANALYTICS REPORT", pdfW/2, 114, {align:"center"});
      // Dept & Month
      pdf.setFontSize(13); pdf.setFont("helvetica","bold");
      pdf.setTextColor(129,140,248);
      pdf.text(`Department: ${dept.charAt(0).toUpperCase()+dept.slice(1)}`, pdfW/2, 135, {align:"center"});
      pdf.text(`Period: ${month==="all"?"All Months":month}`, pdfW/2, 145, {align:"center"});
      // KPI summary on cover
      pdf.setFontSize(10); pdf.setFont("helvetica","normal");
      const kpiLines = [
        `Total Check: ${fmt(T.check)}`,
        `QC Pass: ${fmt(T.pass)}`,
        `Reject Qty: ${fmt(T.rej)}`,
        `Reject Rate: ${T.rejPct.toFixed(2)}%`,
        `Total Defect: ${fmt(T.def)}`,
        `Defect Rate: ${T.defPct.toFixed(2)}%`,
      ];
      pdf.setFillColor(15,23,42);
      pdf.roundedRect(20, 160, pdfW-40, 80, 4, 4, "F");
      kpiLines.forEach((line,i)=>{
        const x = i%2===0 ? 35 : pdfW/2+10;
        const y = 178 + Math.floor(i/2)*18;
        pdf.setTextColor(i%2===0?59:130, i%2===0?130:185, i%2===0?246:129);
        pdf.setFontSize(9); pdf.setFont("helvetica","bold");
        pdf.text(line.split(":")[0]+":", x, y);
        pdf.setTextColor(226,232,240);
        pdf.text(line.split(":")[1].trim(), x+36, y);
      });
      // Footer
      pdf.setFontSize(8); pdf.setTextColor(71,85,105);
      pdf.text(`Generated: ${new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"long",year:"numeric"})}`, pdfW/2, pdfH-10, {align:"center"});
      pdf.text("Amantex QA System v3.2", pdfW/2, pdfH-5, {align:"center"});

      // Section pages
      for (let i=0; i<sections.length; i++) {
        const {ref, title} = sections[i];
        ...
        pdf.rect(0,0,pdfW,pdfH,"F");
        pdf.setFillColor(99,102,241);
        pdf.rect(0,0,pdfW,2,"F");
        pdf.setFontSize(14); pdf.setFont("helvetica","bold");
        pdf.setTextColor(226,232,240);
        pdf.text(title, 14, 16);
        pdf.setFontSize(8); pdf.setFont("helvetica","normal");
        pdf.setTextColor(100,116,139);
        pdf.text(`${dept.charAt(0).toUpperCase()+dept.slice(1)} · ${month==="all"?"All Months":month}`, 14, 23);
        ...
        pdf.addImage(imgData, "PNG", 14, 30, imgW, finalH);

        // Page number
        pdf.setFontSize(8); pdf.setTextColor(71,85,105);
        pdf.text(`${i+2}`, pdfW-14, pdfH-5, {align:"right"});
      }

      pdf.save(`Amantex-${dept}-${month}-Report.pdf`);
    } catch(e) {
      console.error(e);
  ...
  if(!sheets) return <UploadScreen onFile={handleFile} onGSheet={handleGSheet} dark={dark} onToggle={()=>setDark(!dark)}/>;
    ...
    <div style={{minHeight:"100vh",background:bg.page,color:bg.text,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:860,margin:"0 auto",padding:"10px 10px 40px"}}>
        ...
        <div style={{...card,padding:"12px 14px",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:5,flexWrap:"wrap",justifyContent:"flex-end"}}>
              ...
              {/* PDF */}
              <button onClick={exportPDF} disabled={pdfLoading} style={{padding:"6px 9px",borderRadius:8,border:"1px solid rgba(244,63,94,0.3)",background:"rgba(244,63,94,0.1)",color:"#f43f5e",cursor:pdfLoading?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:3,fontSize:10,fontWeight:600,opacity:pdfLoading?0.7:1}}>
                {pdfLoading
                  ?<><Loader size={10} style={{animation:"spin 1s linear infinite"}}/> তৈরি হচ্ছে...</>
          ...
          <div style={{display:"flex",gap:4,marginBottom:8}}>
            {["embroidery","printing"].map(d=>(
              <button key={d} onClick={()=>{setDept(d);setMonth("all");}} style={{flex:1,padding:"8px",borderRadius:10,border:"none",cursor:"pointer",fontSize:12,fontWeight:700,background:dept===d?"linear-gradient(135deg,#6366f1,#8b5cf6)":"transparent",color:dept===d?"#fff":bg.sub,display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
        ...
        {data.length===0?(
          ...
          <>
            {/* KPI */}
            <div ref={refKPI} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
              {kpis.map(({title,val,icon:Icon,color,k})=>(
                <div key={title} style={{...card,padding:"11px 11px 9px",position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:3}}>
                    <p style={{fontSize:9,fontWeight:600,color:bg.sub,textTransform:"uppercase",letterSpacing:"0.05em",margin:0}}>{title}</p>
            ...
            {/* Daily Chart */}
            <div ref={refDaily} style={{...card,padding:"14px 14px 16px",marginBottom:10}}>
            ...
            {/* Gauge */}
            <div ref={refGauge} style={{...card,padding:"14px 14px 12px",marginBottom:10}}>
            ...
            {/* Defect Bar */}
            <div ref={refDefect} style={{...card,padding:"14px 14px 16px",marginBottom:10}}>
            ...
            {/* Pie */}
            <div ref={refPie} style={{...card,padding:"14px 14px 12px",marginBottom:10}}>
            ...
            {/* Top 5 Table */}
            <div ref={refTable} style={{...card,padding:"14px 14px",marginBottom:10}}>
            ...
            <div style={{...card,padding:"10px 14px",display:"flex",flexWrap:"wrap",gap:12,alignItems:"center",justifyContent:"space-between"}}>
              ...
              <div style={{fontSize:9,color:bg.sub}}>Amantex QA v3.2</div>
            </div>
          </>
        )}
      </div>
