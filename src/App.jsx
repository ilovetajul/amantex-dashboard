...
const formatDate = val => {
  ...
  if (val instanceof Date) {
    return `${String(val.getDate()).padStart(2,'0')}-${val.toLocaleString('en',{month:'short'})}`;
  }
  // Handle "01.01.2026" format
...
// Parse Amantex specific sheet format
// Row 0-2: company info, Row 3: headers, Row 4: sub-headers, Row 5+: data
const parseAmantexSheet = (worksheet) => {
  ...
  for (let i = 5; i < rows.length; i++) {
    ...
    const reject  = Number(row[3]) || (check - pass);
    ...
    result.push({ date, totalCheck: check, qcPass: pass, reject, defect });
  }
  return result;
};
...
const CTip = ({ active, payload, label, dark }) => {
  ...
  return (
    <div style={{ background: dark?"rgba(15,23,42,0.97)":"rgba(255,255,255,0.97)", border:"1px solid rgba(99,102,241,0.3)", borderRadius:12, padding:"10px 14px", backdropFilter:"blur(12px)", boxShadow:"0 8px 32px rgba(0,0,0,0.18)" }}>
  ...
  return (
    <div style={{ position:"relative", display:"flex", flexDirection:"column", alignItems:"center" }}>
      ...
      <div style={{ position:"absolute", bottom:4, textAlign:"center" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:4, marginTop:3, fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:20, background:good?"rgba(16,185,129,0.15)":"rgba(244,63,94,0.15)", color:fill }}>
    ...
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:bgPage, padding:24 }}>
      <div style={{ maxWidth:540, width:"100%", textAlign:"center" }}>
        ...
        <div
          ...
          style={{ border:`2px dashed ${drag?"#6366f1":dark?"rgba(99,102,241,0.4)":"rgba(99,102,241,0.3)"}`, borderRadius:20, padding:"44px 28px", background:drag?(dark?"rgba(99,102,241,0.12)":"rgba(99,102,241,0.06)"):(dark?"rgba(15,23,42,0.6)":"rgba(255,255,255,0.8)"), backdropFilter:"blur(20px)", cursor:"pointer", transition:"all 0.2s", boxShadow:dark?"0 4px 32px rgba(0,0,0,0.4)":"0 4px 32px rgba(99,102,241,0.08)" }}>
          ...
          <p style={{ fontWeight:800, fontSize:18, color:dark?"#e2e8f0":"#0f172a", marginBottom:6 }}>Excel ফাইল আপলোড করুন</p>
          <p style={{ fontSize:13, color:"#64748b", marginBottom:16 }}>ড্র্যাগ করুন অথবা ক্লিক করুন</p>
        ...
        <input id="xl-inp" type="file" accept=".xlsx,.xls" style={{ display:"none" }} onChange={handle}/>
        ...
        <div style={{ marginTop:20, padding:"16px 20px", background:bgCard, borderRadius:14, border:"1px solid rgba(99,102,241,0.18)", backdropFilter:"blur(12px)", textAlign:"left" }}>
          <p style={{ fontSize:11, fontWeight:700, color:"#6366f1", marginBottom:10, textTransform:"uppercase", letterSpacing:"0.06em" }}>✅ আপনার ফাইল সাপোর্ট করে</p>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            {[
              {sheet:"EMB-January26",     icon:"🧵"},
              {sheet:"EMB-February",      icon:"🧵"},
              {sheet:"Printing January26",icon:"🖨️"},
              {sheet:"Printing February26",icon:"🖨️"},
            ].map(({sheet,icon})=>(
              <div key={sheet} style={{ background:dark?"rgba(99,102,241,0.1)":"rgba(99,102,241,0.06)", borderRadius:8, padding:"6px 10px", border:"1px solid rgba(99,102,241,0.15)", fontSize:11, color:dark?"#e2e8f0":"#1e293b", fontWeight:600 }}>
                {icon} {sheet}
              </div>
            ))}
          </div>
...
export default function App() {
  ...
  const card = { background:bg.card, border:`1px solid ${bg.border}`, borderRadius:20, backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", boxShadow:dark?"0 4px 32px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.04)":"0 4px 32px rgba(99,102,241,0.08),inset 0 1px 0 rgba(255,255,255,0.8)" };
  ...
  const handleFile = useCallback(file => {
    ...
    reader.onload = e => {
      try {
        ...
        // Group sheets by department
        const embSheets   = names.filter(n => n.toLowerCase().includes("emb"));
        const printSheets = names.filter(n => n.toLowerCase().includes("print"));

        // Parse each sheet and store with month label
        const embData   = {};
        const printData = {};

        embSheets.forEach(n => {
          const label = n.includes("January") || n.includes("Jan") ? "January" :
                        n.includes("February") || n.includes("Feb") ? "February" :
                        n.includes("March") || n.includes("Mar") ? "March" : n;
          const data = parseAmantexSheet(wb.Sheets[n]);
          if (data.length) embData[label] = data;
        });

        printSheets.forEach(n => {
          const label = n.includes("January") || n.includes("Jan") ? "January" :
                        n.includes("February") || n.includes("Feb") ? "February" :
                        n.includes("March") || n.includes("Mar") ? "March" : n;
          const data = parseAmantexSheet(wb.Sheets[n]);
          if (data.length) printData[label] = data;
        });

        if (!Object.keys(embData).length && !Object.keys(printData).length) {
          setError("ডেটা পাওয়া যায়নি!"); return;
        }

        // Combine all months for "all" view
        const combineAll = obj => Object.values(obj).flat();
        ...
        setSheets({
          embroidery: combineAll(embData),
          printing:   combineAll(printData),
        });
        setMonth("all");
        setFileName(file.name);
      } catch(err) {
        setError("ফাইল পড়তে সমস্যা: " + err.message);
      }
    };
    reader.readAsArrayBuffer(file);
  }, []);

  // Get data based on dept + month filter
  const data = useMemo(() => {
    if (!sheets) return [];
  ...
  const availableMonths = useMemo(() => {
    if (!allSheets) return [];
    return Object.keys(allSheets[dept] ?? {});
  }, [allSheets, dept]);
  ...
  const exportCSV = () => {
    ...
    const a    = Object.assign(document.createElement("a"),{href:url,download:`${dept}-${month}-report.csv`});
  ...
  const btnStyle = (color, active=true) => ({
    display:"flex", alignItems:"center", gap:5, padding:"7px 13px", borderRadius:9,
    border:`1px solid ${color}40`, cursor:"pointer", fontSize:12, fontWeight:600,
    background:active?`${color}18`:"transparent", color,
  });
        ...
        <div style={{ ...card, padding:"14px 20px", marginBottom:16, display:"flex", flexWrap:"wrap", alignItems:"center", gap:12 }}>
          <div style={{ display:"flex", alignItems:"center", gap:11, flex:1, minWidth:160 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:bg.sub, fontWeight:600, textTransform:"uppercase", letterSpacing:"0.06em" }}>{fileName}</div>
            </div>
          </div>
          ...
          <div style={{ display:"flex", gap:3, background:dark?"rgba(30,41,59,0.8)":"rgba(241,245,249,0.9)", padding:3, borderRadius:11, border:`1px solid ${bg.border}` }}>
            {["embroidery","printing"].map(d=>(
              <button key={d} onClick={()=>{setDept(d);setMonth("all");}} style={{ padding:"6px 14px", borderRadius:8, border:"none", cursor:"pointer", fontSize:12, fontWeight:600, transition:"all 0.2s", background:dept===d?(dark?"rgba(99,102,241,0.9)":"#6366f1"):"transparent", color:dept===d?"#fff":bg.sub, display:"flex", alignItems:"center", gap:5 }}>
          ...
          <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
            {["all",...availableMonths].map(m=>(
              <button key={m} onClick={()=>setMonth(m)} style={{ padding:"6px 12px", borderRadius:8, border:`1px solid ${month===m?"#6366f1":"rgba(99,102,241,0.2)"}`, cursor:"pointer", fontSize:11, fontWeight:600, background:month===m?"rgba(99,102,241,0.2)":"transparent", color:month===m?"#818cf8":bg.sub }}>
                {m==="all"?"সব মাস":m}
              </button>
            ))}
          </div>
          ...
          <div style={{ display:"flex", gap:7 }}>
            <button onClick={exportCSV} style={btnStyle("#10b981")}><Download size={12}/> CSV</button>
            <button onClick={()=>setSheets(null)} style={btnStyle("#818cf8")}><RefreshCw size={12}/> নতুন ফাইল</button>
            <button onClick={()=>setDark(!dark)} style={btnStyle(dark?"#fbbf24":"#6366f1")}>
              {dark?<Sun size={12}/>:<Moon size={12}/>}{dark?"Light":"Dark"}
            </button>
          </div>
        </div>

        {error&&<div style={{ ...card, padding:"12px 18px", marginBottom:14, background:"rgba(244,63,94,0.12)", border:"1px solid rgba(244,63,94,0.3)", color:"#f43f5e", fontSize:13, fontWeight:600 }}>⚠️ {error}</div>}

        {data.length===0?(
          <div style={{ ...card, padding:40, textAlign:"center", color:bg.sub }}>
            <p style={{ fontSize:15, fontWeight:600 }}>কোনো ডেটা নেই।</p>
          </div>
        ):(
          <>
            {/* KPI */}
              ...
              {kpis.map(({title,val,icon:Icon,color,k})=>(
                <div key={title} style={{ ...card, padding:"16px 16px", position:"relative", overflow:"hidden" }}>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
                    <p style={{ fontSize:10, fontWeight:600, color:bg.sub, textTransform:"uppercase", letterSpacing:"0.06em", margin:0 }}>{title}</p>
                    <div style={{ background:`${color}18`, borderRadius:8, padding:6, border:`1px solid ${color}25` }}><Icon size={14} color={color}/></div>
                  </div>
                  <div style={{ fontFamily:"DM Mono,monospace", fontSize:24, fontWeight:700, color:bg.text, lineHeight:1, marginBottom:3 }}>{val}</div>
                  <div style={{ height:26, margin:"3px 0" }}><Spark data={data.slice(-12)} color={color} k={k}/></div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 270px", gap:13, marginBottom:16 }}>
              <div style={{ ...card, padding:"18px 20px" }}>
                ...
                <p style={{ margin:"0 0 14px", fontSize:11, color:bg.sub }}>Check · Pass · Reject — প্রতিদিনের তথ্য · {month==="all"?"সব মাস":month}</p>
              ...
              <div style={{ ...card, padding:"18px 16px", display:"flex", flexDirection:"column" }}>
                ...
                <div style={{ borderTop:`1px solid ${bg.border}`, paddingTop:10, marginTop:4 }}>
                  ...
                  <div style={{ height:5, background:dark?"rgba(51,65,85,0.5)":"rgba(226,232,240,0.8)", borderRadius:10, overflow:"hidden", marginTop:2 }}>
                    <div style={{ height:"100%", width:`${Math.min((totals.rejPct/1.0)*100,100)}%`, background:totals.rejPct<0.5?"linear-gradient(90deg,#10b981,#34d399)":"linear-gradient(90deg,#f43f5e,#fb7185)", borderRadius:10 }}/>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div style={{ ...card, padding:"18px 20px" }}>
                ...
                <div style={{ display:"flex", height:250, alignItems:"center" }}>
                  <div style={{ flex:1, height:"100%" }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={[{name:"QC Pass",value:totals.pass},{name:"Rejected",value:totals.rej},{name:"Defect",value:totals.def}]}
            ...
            <div style={{ ...card, padding:"18px 20px", marginBottom:16 }}>
              ...
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                  <thead>
                    <tr>{["তারিখ","Total Check","QC Pass","Reject","Reject %","Defect"].map(h=>(
            ...
            <div style={{ ...card, padding:"11px 20px", display:"flex", flexWrap:"wrap", gap:14, alignItems:"center", justifyContent:"space-between" }}>
              <div style={{ display:"flex", gap:18, flexWrap:"wrap" }}>
                {[["ফাইল",fileName||"—","#6366f1"],["Department",dept.charAt(0).toUpperCase()+dept.slice(1),"#8b5cf6"],["Month",month==="all"?"All":month,"#3b82f6"],["Grade",totals.rejPct<0.5?"A+ ✓":"B",totals.rejPct<0.5?"#10b981":"#f97316"]].map(([l,v,c])=>(
                  <div key={l}>
              ...
              <div style={{ fontSize:10, color:bg.sub }}>Amantex QA System v3.0 · {new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})}</div>
            </div>
          </>
        )}
      </div>
